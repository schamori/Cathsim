"""
Create AAA002 phantom from STL file with centerline-based sites
"""
...
def load_centerline_endpoints(centerlines_dir: str, phantom_name: str):
    ...
    Args:
        ...
        phantom_name: Name of the phantom (e.g., "AAA002")

    Returns:
        dict with 'start' and 'goal' positions
    """
    if pv is None:
        print("Warning: Cannot load centerlines without pyvista. Using default positions.")
        return {"start": [0.0, 0.0, 0.0], "goal": [0.0, 0.0, 0.15]}
    ...
    try:
        ...
        if aorta_path.exists():
            ...
            # Get the last point as the goal
            goal_point = aorta.points[-1]
            # Convert to meters (assuming centerlines are in mm)
            sites['goal'] = (goal_point / 1000.0).tolist()
            print(f"[OK] Goal site from aorta: {sites['goal']}")
        else:
...
def create_aaa_phantom(stl_path: str, phantom_name: str, centerlines_dir: str = None):
    ...
    Args:
        ...
        phantom_name: Name for the phantom (e.g., "AAA002")
        centerlines_dir: Directory containing centerline VTP files
    """
...
def generate_xml(phantom_name: str, num_hulls: int, sites: dict = None) -> str:
    ...
    # Generate mesh references (scale will be set via default mesh scale)
...
if __name__ == "__main__":
    # Create AAA002 phantom with centerline sites
    create_aaa_phantom(
        stl_path="C:/Users/Admin/Downloads/10932957/meshes/meshes/AAA002.stl",
        phantom_name="AAA002",
